...
type Rule<'input, 'output> = ParseError ref -> 'input -> ParseResult<'input, 'output>
...
let Map (binding: 'o1 -> 'o2) (rule: Rule<'i, 'o1>) : Rule<'i, 'o2> = fun err ->
  rule err >> ParseResultMap binding
let (<-/) = Map
let (->/) r f = Map f r

let Series (rule1: Rule<'i, 'o1>) (rule2: Rule<'i, 'o2>) : Rule<'i, 'o1 * 'o2> = fun err input ->
  rule1 err input
   |> Option.bind (fun (input', res1) -> ParseResultMap (mkpair res1) (rule2 err input'))
let (+/) = Series
type SequenceBuilder = SequenceOf
  with
    member __.Bind(rule: Rule<'i, 'o1>, cont: 'o1 -> Rule<'i, 'o2>) : Rule<'i, 'o2> = fun err input ->
      match rule err input with
      |Some(input', x) -> cont x err input'
      |None -> None
    member __.Return(x: 'o) : Rule<'i, 'o> = fun err input -> Some(input, x)

let Parallel (rule1: Rule<'i, 'o>) (rule2: Rule<'i, 'o>) : Rule<'i, 'o> = fun err input ->
  match rule1 err input with
  |Some res -> Some res
  |None -> rule2 err input
let (|/) = Parallel
let OneOf (rules: Rule<'i, 'o> list) : Rule<'i, 'o> = List.reduce (|/) rules

let Optional (rule: Rule<'i, 'o>) : Rule<'i, 'o option> = fun err input ->
  match rule err input with
...
let OptionalListOf rule : Rule<'i, 'o list> = FoldBackListOf (fun a b -> a::b) rule (fun err i -> Some(i, []))
...
let LookAhead (rule: Rule<'i, 'o>) : Rule<'i, 'o> = fun err input ->
  Option.map (fun (i, o) -> (input, o)) (rule err input)
let (&/) rule1 rule2 = LookAhead rule1 +/ rule2 ->/ snd

let inline Equal (token: string) : Rule< ^I, 'o> = fun err input ->
...
let inline Match (rgx: string) : Rule< ^I, 'o> = fun err input ->
...
let inline run_parser (parse_rule: Rule< ^I, 'o>) (input: ^I) : 'o =
  ...
  match parse_rule error input with
  |Some(rest, result) ->
