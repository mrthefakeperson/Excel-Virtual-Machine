...
let rec cast dt ast = Apply(Value(Var("\cast", dt)), [ast])
...
let rec check_type (symtbl: SymbolTable) = function
  ...
  |Apply(f, args) ->
    ...
    |Value(Var(fname, _)), (DT.Function(_, ret_type) as ftype) ->
      ...
      match DT.infer_type ftype ftype' with
      |DT.Function(inferred_arg_types, inferred_ret_type) as ftype ->
        let args'' = 
          List.map3 (fun arg expected_type discovered_type ->
            if expected_type = discovered_type then arg
            else cast (DT.infer_type expected_type discovered_type) arg
           ) args' inferred_arg_types discovered_arg_types
        Apply(Value(Var(fname, ftype)), args''), inferred_ret_type
      |_ -> failwith "should never be reached"
    |f', DT.Function2 ret_type -> Apply(f', args'), ret_type
  ...
  |If(cond, thn, els) ->
    ...
    let cond'' = if condt = Byte then cond' else cast Byte cond'
  ...
  |Function(args, body) ->
    ...
    Function(args, body'), DT.Function(arg_types, TypeClasses.any)
  |GlobalParse xprs ->
